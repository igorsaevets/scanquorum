# scanquorum/engines/rapidocr_multi.py
"""RapidOCR (PP-OCR family) -- multilingual v6 medium recogniser. Best SINGLE layer measured: 95.9 % against 91.5 % for the existing text layer.
...
PARAMS = dict(_DET, **{"Rec.ocr_version": OCRVersion.PPOCRV6, "Rec.model_type": ModelType.MEDIUM})
...
def run(pdf, dpi=300, progress=None):
    ...
    for pi in range(doc.page_count):
        ...
        if progress:
            progress(pi + 1, doc.page_count, "rapidocr_multi")
    doc.close()
    return out
