...
The expensive failure in document processing is not a wrong character. It is a wrong
character that nothing downstream can distinguish from a right one.
...
records that it applies majority voting with heuristics to break ties. That sentence is
worth pausing on, because it means the problem in Section~\ref{sec:audit} was visible in
1996 to anyone who asked what the heuristics were.
...
\texttt{rapidocr\_multi} & 78.6\% & [66.2, 87.3] \\
\texttt{rapidocr\_en} & 60.4\% & [46.9, 72.4] \\
\textbf{the ensemble} & \textbf{94.6\%} & \textbf{[85.4, 98.2]} \\
\bottomrule
\end{tabular}
\caption{Each voter and the ensemble against a human reading of the page, on 56 scored
positions. Intervals are Wilson. The sample is stratified toward contested decisions, so
these are not document level accuracies and must not be read as such.}
\label{tab:voters}
\end{table}
...
neural networks. In \emph{Advances in Neural Information Processing Systems}, 2017.
